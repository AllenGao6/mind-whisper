...
function createSession({ providerId, providersConfig, hooks }) {
  const cfg = providersConfig?.[providerId];
  if (!cfg) throw new ProviderConfigError(`Unknown provider: ${providerId}`);
  switch (providerId) {
    case 'openai':
      return openai.createSession({ apiKey: cfg.apiKey, hooks });
    case 'groq':
      return groq.createSession({ apiKey: cfg.apiKey, hooks });
    case 'deepgram':
